...
void DvaOdin(int *pt, int len)
{
	...
	if (i1 == 0 && i2 == 0)
	{
		printf("Элементы одинаковы");
	}
	else
	{
		printf("Элменты разные %d ", i1, i2);
	}
}
...
void srednee(int *pt, int len, int len2)
{
	...
	for (int j = 0; j < len2; j++)
	{
		...
		printf("среднее арифметическое каждого столбца =%d\n", len2, summ / len);
		summ = 0;
	}
}
...
void MaxMin(int *pt, int len, int len2)
{
	...
	for (int j = 0; j < len; j++)
	{
		...
		printf("минимум =%d  максимум =%d  %d строки\n", min, max, j);
		max = 0;
		min = 9999;
	}
}
void chet(int *pt, int len, int len2)
{
	...
	printf("количество четных  в массиве-%d нечетных чисел-%d", Nechetsumm, chetsumm);
}
void element7(int *pt, int len, int len2)
{
	...
	printf("сколько раз встречается число 7 среди элементов массива 7 =%d", chetsumm);
}
void Min(int *pt, int len, int len2)
{
	...
	printf(" индексы первого наименьшего  элемента массива =%d  ", min);
}